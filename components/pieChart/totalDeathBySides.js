...
export default function TotalDeathBySides(props) {
    //Initial variable
    ...
    function getSeries(val){
        let catSeries = [];
        val.forEach(e => { 
            if(props.source == "Military"){
                catSeries.push(parseInt(e.m_death));
            } else {
                catSeries.push(parseInt(e.c_death));
            }
        });
        return catSeries;
    }
    ...
    if (error) {
        ...
        return (
            <div className='custom-tbody' style={{padding:"6px"}}>
                <h6>Total {props.source} Death By Sides</h6>
