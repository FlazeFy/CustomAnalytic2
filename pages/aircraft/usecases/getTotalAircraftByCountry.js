...
export default function GetTotalAircraftByCountry({ctx}) {
    //Initial variable
    const [error, setError] = useState(null)
    const [isLoaded, setIsLoaded] = useState(false)
    const [items, setItems] = useState([])
    const filter_name = "Aircraft_Country"
    ...
    useEffect(() => {
        ...
        fetch(`https://ww2-test.leonardhors.com/api/aircraft/total/bycountry/${keyLimit}`)
        .then(res => res.json())
            .then(
            (result) => {
                setIsLoaded(true)
