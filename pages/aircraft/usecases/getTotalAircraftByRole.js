...
export default function GetTotalAircraftByRole({ctx}) {
    //Initial variable
    const [error, setError] = useState(null)
    const [isLoaded, setIsLoaded] = useState(false)
    const [items, setItems] = useState([])
    const filter_name = "Aircraft_Role"
    ...
    useEffect(() => {
        ...
        fetch(`https://ww2-test.leonardhors.com/api/aircraft/total/byrole/${keyLimit}`)
        .then(res => res.json())
            .then(
            (result) => {
                setIsLoaded(true)
