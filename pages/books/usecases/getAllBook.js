...
export default function GetAllBook({ctx}) {
    ...
    useEffect(() => {
        ...
        fetch(`https://ww2-test.leonardhors.com/api/books/limit/${keyLimit}/order/${keyOrder}/find/${keySearch}?page=${keyPage}`)
        .then(res => res.json())
            .then(
            (result) => {
                setIsLoaded(true)
    ...
    if (error) {
        ...
        return (
            <> 
                ...
                <MoleculesTable builder={builder} items={items} maxPage={maxPage} currentPage={currPage} ctx={"Books"}/>  
            </>
        )
    }
}
