...
export default function GetTotalFacilityByType({ctx}) {
    //Initial variable
    const [error, setError] = useState(null)
    const [isLoaded, setIsLoaded] = useState(false)
    const [items, setItems] = useState([])
    const filter_name = "Facility_Type"
    ...
    useEffect(() => {
        ...
        fetch(`https://ww2-test.leonardhors.com/api/facilities/total/bytype/${keyLimit}`)
        .then(res => res.json())
            .then(
            (result) => {
                setIsLoaded(true)
