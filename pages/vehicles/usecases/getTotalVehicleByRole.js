...
export default function GetTotalVehicleByRole({ctx}) {
    //Initial variable
    const [error, setError] = useState(null)
    const [isLoaded, setIsLoaded] = useState(false)
    const [items, setItems] = useState([])
    const filter_name = "Vehicle_Role"
    ...
    useEffect(() => {
        ...
        fetch(`https://ww2-test.leonardhors.com/api/vehicles/total/byrole/${keyLimit}`)
        .then(res => res.json())
            .then(
            (result) => {
                setIsLoaded(true)
